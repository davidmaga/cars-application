import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { of } from 'rxjs';

import { GearboxService } from '../service/gearbox.service';

import { GearboxComponent } from './gearbox.component';

describe('Gearbox Management Component', () => {
  let comp: GearboxComponent;
  let fixture: ComponentFixture<GearboxComponent>;
  let service: GearboxService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      declarations: [GearboxComponent],
    })
      .overrideTemplate(GearboxComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(GearboxComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(GearboxService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.gearboxes?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });
});
